...
if R>0 and R<70:#проверка на радиус
    figu = plt.figure()
    ax = plt.axes()
    ...
    if R>10 and R<30:
        ax.set(xlim=(-30, 40), ylim=(-30, 40))
    elif R>30 and R<70:
        ax.set(xlim=(-75, 80), ylim=(-80, 80))
    else:
        ax.set(xlim=(-10, 20), ylim=(-15, 15))  # устанавливает ограничения для осей x и y, чтобы значения на графике были видны в заданных пределах.5
    ax.grid(color='blue', linewidth=0.5,
            linestyle='dotted')  # добавляет на график сетку синего цвета, толщиной 0.5 и пунктирным стилем.
    ...
    ax.xaxis.set_major_locator(plt.MultipleLocator(5))  # устанавливает основной локатор на оси X, который размещает маркеры на основе заданного интервала, маркеры будут размещены каждые 2 единицы на оси X.
